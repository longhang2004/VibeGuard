...
export class AuthService {
  ...
  async refresh(refreshDto: RefreshDto) {
    ...
    // 1. Verify token signature — throws if invalid
    ...
    // 2. Check blacklist in Redis (outside try/catch so UnauthorizedException propagates)
    ...
    const accessToken = this.jwtService.sign(
      { sub: payload.sub, email: payload.email },
      {
        ...
        expiresIn: process.env.GATEWAY_JWT_ACCESS_EXPIRATION || '15m',
      } as any,
    );
  ...
  private generateTokens(userId: string, email: string) {
    ...
    const refreshToken = this.jwtService.sign(payload, {
      secret: process.env.GATEWAY_JWT_REFRESH_SECRET || 'dev_jwt_refresh_secret_key_vibeguard_67890',
      expiresIn: process.env.GATEWAY_JWT_REFRESH_EXPIRATION || '7d',
    } as any);
